...
def CreateAccount(UserName, userEmail, userAccountType, InitialBalance):
    ...
    print(
        f"Account {accountNumber} has been created with an initial balance of {InitialBalance} {userAccountType} for {UserName} at {transactionRecipient}"
    )
...
def GetBalance(accountNumber):
    return (
        pyrebase_db.child("Account").child(accountNumber).child("balance").get().val()
    )
...
def CreateTransaction(accountNumber, transactionType, transactionAmount):
    ...
    pyrebase_db.child("Account").child(accountNumber).child("transactions").push(
        transaction_data
    )
...
def GetTransactions(accountNumber):
    ...
    try:
        for transaction in transactions:
            ...
            print(
                f" {transaction.val().get('recipient')} {transaction.val().get('type')} {transaction.val().get('amount')} "
            )
            print("")
    except:
        print("No transactions found")
