...
def generate_llama2_response(prompt_input, context="", db_ans=""):
    ...
    if context != "":
        string_dialogue = answer_using_context_template.format(
            chat_history=chat_history, user_msg=prompt_input, context=context
        )
    elif db_ans != "":
        string_dialogue = answer_using_database_answer_template.format(
            chat_history=chat_history, user_msg=prompt_input, db_ans=db_ans
        )
    else:
        string_dialogue = answer_using_llm.format(
            chat_history=chat_history, user_msg=prompt_input
        )
